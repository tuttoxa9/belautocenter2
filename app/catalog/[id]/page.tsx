...
// Используем динамический рендеринг с ревалидацией
export const dynamicParams = true
export const revalidate = 3600 // Ревалидация данных каждый час

// Функция для парсинга данных Firestore (остается без изменений)
...
const convertFieldValue = (value: any): any => {
  ...
  if (value.arrayValue !== undefined) return value.arrayValue.values?.map((v: any) => convertFieldValue(v)) || []
...
// Выносим получение данных в отдельную функцию, чтобы избежать дублирования
async function getCarData(id: string) {
  try {
    ...
    const response = await fetch(firestoreUrl, {
      ...
      // Используем встроенное кэширование Next.js
    ...
    // Добавляем ID к объекту, так как он не приходит из Firestore
    if (carData) {
      carData.id = id
    }
    ...
    console.error(`Failed to fetch car data for ID ${id}:`, error)
    return null
  }
}
...
export default async function CarDetailsPage({ params }: { params: { id: string } }) {
  ...
  // Передаем данные напрямую в клиентский компонент
  return <CarDetailsClient initialCar={carData} carId={params.id} />
}
