...
import {
  Gauge, Fuel, Settings, Car, Phone, ChevronLeft, ChevronRight,
  Calculator, Eye, Clock, AlertCircle, Check
} from "lucide-react"
...
export default function CarDetailsClient({ carId }: CarDetailsClientProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (car?.imageUrls && car.imageUrls.length > 1) {
        if (event.key === 'ArrowLeft') setFullscreenImageIndex(p => (p - 1 + car.imageUrls.length) % car.imageUrls.length)
        if (event.key === 'ArrowRight') setFullscreenImageIndex(p => (p + 1) % car.imageUrls.length)
      }
    }
    if (isFullscreenOpen) document.body.style.overflow = 'hidden'
  ...
  const navigateToImage = (index: number) => {
    if (!car?.imageUrls || index < 0 || index >= car.imageUrls.length) return;
    setCurrentImageIndex(index);
    if (galleryScrollRef.current) {
      galleryScrollRef.current.scrollTo({ left: index * galleryScrollRef.current.clientWidth, behavior: 'smooth' });
    }
  };

  const navigatePrevious = () => navigateToImage(currentImageIndex - 1);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      <div className="container mx-auto px-1 sm:px-2 lg:px-4 py-4 sm:py-6 max-w-7xl">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
              <div className="lg:col-span-8 lg:border-r border-slate-200/50">
                {/* Gallery, videos, and description */}
              </div>
        ...
        {/* Fullscreen gallery and other dialogs */}
      </div>
    </div>
  )
}