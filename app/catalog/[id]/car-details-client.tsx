...
export default function CarDetailsClient({ carId }: CarDetailsClientProps) {
  ...
  const [actionDrawerMode, setActionDrawerMode] = useState<'booking' | 'callback'>('booking')
  ...
  const openActionDrawer = (mode: 'booking' | 'callback') => {
  ...
  const loadStaticData = async () => {
    try {
      ...
      const contactsUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/pages/contacts`
      ...
      staticDataCache.lastLoadTime = now
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      <div className="container mx-auto px-1 sm:px-2 lg:px-4 py-4 sm:py-6 max-w-7xl">
              ...
              <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
                ...
                <div className="pt-3 sm:pt-4 border-t border-slate-200/50">
                  <div className="grid grid-cols-1 gap-2 sm:gap-3">
                    <Button onClick={() => openActionDrawer('booking')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl py-2 sm:py-3 text-sm sm:text-base">
                      <Eye className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />
                      Записаться на просмотр
                    </Button>
                    <Button onClick={() => openActionDrawer('callback')} className="w-full bg-white hover:bg-slate-50 text-slate-900 border-2 border-slate-200 font-semibold rounded-xl py-2 sm:py-3 text-sm sm:text-base">
        ...
        <ActionDrawer
          ...
          title={actionDrawerMode === 'booking' ? 'Записаться на просмотр' : 'Заказать обратный звонок'}
        />
