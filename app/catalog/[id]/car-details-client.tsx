...
import FinancialAssistantDrawer from "@/components/financial-assistant-drawer"
...
export default function CarDetailsClient({ carId }: CarDetailsClientProps) {
  ...
  const loadStaticData = async () => {
    try {
      ...
      if (banks && banks.length > 0) setPartnerBanks(banks)
      else setPartnerBanks([])

      if (leasingCompanies && leasingCompanies.length > 0) setLeasingCompanies(leasingCompanies)
      else setLeasingCompanies([])
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    await bookingButtonState.execute(async () => {
      ...
      await fetch('/api/send-telegram', {
        ...
        body: JSON.stringify({ ...bookingForm, carId: carId, type: 'car_booking' })
      })
  ...
  const handleCallbackSubmit = async (e: React.FormEvent) => {
    ...
    await callbackButtonState.execute(async () => {
      ...
      await fetch('/api/send-telegram', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...callbackForm, carId: carId, type: 'callback' })
      })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      <div className="container mx-auto px-1 sm:px-2 lg:px-4 py-4 sm:py-6 max-w-7xl">
        <nav className="mb-4 sm:mb-6">
          <ol className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-slate-500">
            ...
            <li className="text-slate-900 font-medium px-2 py-1 bg-slate-100 rounded-md">
              {loading ? <div className="h-4 bg-slate-300 rounded w-20 animate-pulse inline-block"></div> : `${car?.make || ''} ${car?.model || ''}`}
            </li>
          </ol>
        </nav>
            ...
            <div className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200/50 p-3 sm:p-6">
              <div className="flex items-start justify-between gap-3 lg:gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                    ...
                    <div className="self-start sm:self-auto">
                      {loading ? <div className="h-6 sm:h-7 bg-slate-300 rounded-full w-16 animate-pulse"></div> : car?.isAvailable ? <div className="bg-green-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold inline-block">В наличии</div> : <div className="bg-red-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold inline-block">Продан</div>}
                    </div>
                  </div>
                ...
                <div className="text-right flex-shrink-0">
                  {loading ? <> <div className="h-6 sm:h-8 lg:h-9 bg-slate-300 rounded w-24 mb-1 animate-pulse ml-auto"></div> <div className="h-4 sm:h-5 lg:h-6 bg-slate-300 rounded w-20 animate-pulse ml-auto"></div> <div className="h-3 sm:h-4 bg-slate-300 rounded w-16 mt-1 animate-pulse ml-auto"></div> </> : <> <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-slate-900 mb-1 leading-tight">{car?.price ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(car.price) : 'Цена по запросу'}</div> {usdBynRate && car?.price && <div className="text-sm sm:text-base lg:text-lg font-semibold text-slate-600">≈ {new Intl.NumberFormat("ru-BY", { style: "currency", currency: "BYN", minimumFractionDigits: 0 }).format(car.price * usdBynRate)}</div>} <div className="text-xs sm:text-sm text-slate-500 mt-1">от {car?.price ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(Math.round(car.price * 0.8 / 60)) : '0'}/мес</div> </>}
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
              ...
              <div className="lg:col-span-4">
                <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
                  ...
                  <div> <h4 className="text-base sm:text-lg font-bold text-slate-900 mb-3">Финансирование</h4> <div className="bg-slate-50 rounded-xl p-3 sm:p-4 border border-slate-200/50"> <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4"> <div className="text-center"> <div className="text-xs sm:text-sm text-slate-500 mb-1">Кредит от</div> {loading ? <div className="h-5 sm:h-6 bg-slate-300 rounded w-16 mx-auto animate-pulse"></div> : <div className="text-sm sm:text-lg font-bold text-slate-900">{car?.price && usdBynRate ? formatPrice(Math.round(car.price * 0.8 / 60), 'USD') : '0'}/мес</div>} </div> <div className="text-center"> <div className="text-xs sm:text-sm text-slate-500 mb-1">Лизинг от</div> {loading ? <div className="h-5 sm:h-6 bg-slate-300 rounded w-16 mx-auto animate-pulse"></div> : <div className="text-sm sm:text-lg font-bold text-slate-900">{car?.price && usdBynRate ? formatPrice(Math.round(car.price * 0.7 / 36), 'USD') : '0'}/мес</div>} </div> </div> <Button onClick={() => setIsFinancialAssistantOpen(true)} className="w-full bg-slate-900 hover:bg-black text-white font-semibold rounded-xl py-2 sm:py-3 text-sm sm:text-base"> <Calculator className="h-3 w-3 sm:h-4 sm:w-4 mr-2" /> Рассчитать кредит/лизинг </Button> </div> </div>
                  <div className="pt-3 sm:pt-4 border-t border-slate-200/50"> <div className="grid grid-cols-1 gap-2 sm:gap-3"> <Dialog open={isBookingOpen} onOpenChange={setIsBookingOpen}> <DialogTrigger asChild> <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl py-2 sm:py-3 text-sm sm:text-base"> <Eye className="h-3 w-3 sm:h-4 sm:w-4 mr-2" /> Записаться на просмотр </Button> </DialogTrigger> </Dialog> <Dialog open={isCallbackOpen} onOpenChange={setIsCallbackOpen}> <DialogTrigger asChild> <Button className="w-full bg-white hover:bg-slate-50 text-slate-900 border-2 border-slate-200 font-semibold rounded-xl py-2 sm:py-3 text-sm sm:text-base"> <Phone className="h-3 w-3 sm:h-4 sm:w-4 mr-2" /> Заказать звонок </Button> </DialogTrigger> </Dialog> </div> </div>
                </div>
              </div>
            </div>
            <div className="bg-gradient-to-r from-blue-600 to-blue-500 text-white p-4 sm:p-6 border-t border-slate-200/50"> <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 items-center"> <div className="text-center md:text-left"> {loading ? <div className="space-y-2"> <div className="h-5 bg-blue-400/40 rounded w-40 mx-auto md:mx-0 animate-pulse"></div> <div className="h-4 bg-blue-400/40 rounded w-52 mx-auto md:mx-0 animate-pulse"></div> </div> : <> <div className="font-medium text-base sm:text-lg mb-1">{settings?.main?.showroomInfo?.companyName || ""}</div> <div className="text-blue-100 text-xs sm:text-sm">{settings?.main?.showroomInfo?.address || ""}</div> </>} </div> <div className="text-center"> {loading ? <div className="h-5 bg-blue-400/40 rounded w-40 mx-auto animate-pulse"></div> : <div className="flex items-center justify-center space-x-2 text-blue-100 text-xs sm:text-sm"> <Clock className="h-3 w-3 sm:h-4 sm:w-4" /> <div><div>{settings?.main?.showroomInfo?.workingHours?.weekdays || ""}</div></div> </div>} </div> <div className="text-center md:text-right"> {loading ? <div className="space-y-2"> <div className="h-5 bg-blue-400/40 rounded w-32 mx-auto md:ml-auto animate-pulse"></div> <div className="h-5 bg-blue-400/40 rounded w-32 mx-auto md:ml-auto animate-pulse"></div> </div> : <div className="flex flex-col items-center md:items-end space-y-1"> {settings?.main?.showroomInfo?.phone && <div className="flex items-center space-x-2"> <Phone className="h-3 w-3 sm:h-4 sm:w-4" /> <div className="font-medium text-base sm:text-lg">{settings.main.showroomInfo.phone}</div> </div>} {settings?.main?.showroomInfo?.phone2 && <div className="flex items-center space-x-2"> <Phone className="h-3 w-3 sm:h-4 sm:w-4" /> <div className="font-medium text-base sm:text-lg">{settings.main.showroomInfo.phone2}</div> </div>} </div>} </div> </div> </div>
          </div>
        </div>

        <FinancialAssistantDrawer
          isOpen={isFinancialAssistantOpen}
          onClose={() => setIsFinancialAssistantOpen(false)}
          car={car}
        ...
        <Dialog open={isBookingOpen} onOpenChange={setIsBookingOpen}> <DialogContent> <DialogHeader> <DialogTitle>Записаться на просмотр</DialogTitle> </DialogHeader> <form onSubmit={handleBookingSubmit} className="space-y-4"> <div> <Label htmlFor="bookingName">Ваше имя</Label> <Input id="bookingName" value={bookingForm.name} onChange={(e) => setBookingForm({ ...bookingForm, name: e.target.value })} required /> </div> <div> <Label htmlFor="bookingPhone">Номер телефона</Label> <div className="relative"> <Input id="bookingPhone" value={bookingForm.phone} onChange={(e) => setBookingForm({ ...bookingForm, phone: formatPhoneNumber(e.target.value) })} placeholder="+375XXXXXXXXX" required className="pr-10" /> {isPhoneValid(bookingForm.phone) && <Check className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-green-500" />} </div> </div> <div> <Label htmlFor="bookingMessage">Комментарий</Label> <Textarea id="bookingMessage" value={bookingForm.message} onChange={(e) => setBookingForm({ ...bookingForm, message: e.target.value })} placeholder="Удобное время для просмотра..." /> </div> <StatusButton type="submit" className="w-full" state={bookingButtonState.state} loadingText="Отправляем..." successText="Заявка отправлена!" errorText="Ошибка"> Записаться на просмотр </StatusButton> </form> </DialogContent> </Dialog>
        <Dialog open={isCallbackOpen} onOpenChange={setIsCallbackOpen}> <DialogContent> <DialogHeader> <DialogTitle>Заказать обратный звонок</DialogTitle> </DialogHeader> <form onSubmit={handleCallbackSubmit} className="space-y-4"> <div> <Label htmlFor="callbackName">Ваше имя</Label> <Input id="callbackName" value={callbackForm.name} onChange={(e) => setCallbackForm({ ...callbackForm, name: e.target.value })} required /> </div> <div> <Label htmlFor="callbackPhone">Номер телефона</Label> <div className="relative"> <Input id="callbackPhone" value={callbackForm.phone} onChange={(e) => setCallbackForm({ ...callbackForm, phone: formatPhoneNumber(e.target.value) })} placeholder="+375XXXXXXXXX" required className="pr-10" /> {isPhoneValid(callbackForm.phone) && <Check className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-green-500" />} </div> </div> <StatusButton type="submit" className="w-full" state={callbackButtonState.state} loadingText="Отправляем..." successText="Заявка отправлена!" errorText="Ошибка"> Заказать звонок </StatusButton> </form> </DialogContent> </Dialog>

        <Dialog open={isFullscreenOpen} onOpenChange={setIsFullscreenOpen}> <DialogContent className="max-w-full h-full p-0 bg-black/95 border-none"> {car?.imageUrls && car.imageUrls.length > 0 && <div className="relative w-full h-full flex items-center justify-center"> <button onClick={() => setIsFullscreenOpen(false)} className="absolute top-4 right-4 z-50 w-12 h-12 bg-black/60 hover:bg-black/80 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95" aria-label="Закрыть"> <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg> </button> <div className="relative w-full h-full flex items-center justify-center select-none" onTouchStart={(e) => { setTouchEnd(null); setTouchStart(e.targetTouches[0].clientX); }} onTouchMove={(e) => { if (!touchStart) return; setTouchEnd(e.targetTouches[0].clientX); }} onTouchEnd={() => { if (!touchStart || !touchEnd) return; const distance = touchStart - touchEnd; const isLeftSwipe = distance > 50; const isRightSwipe = distance < -50; if (isLeftSwipe && car.imageUrls.length > 1) { setFullscreenImageIndex((prev) => (prev + 1) % car.imageUrls.length); } if (isRightSwipe && car.imageUrls.length > 1) { setFullscreenImageIndex((prev) => (prev - 1 + car.imageUrls.length) % car.imageUrls.length); } }} > <Image src={getCachedImageUrl(car.imageUrls[fullscreenImageIndex])} alt={`${car?.make} ${car?.model} - фото ${fullscreenImageIndex + 1}`} fill className="object-contain" priority quality={90} sizes="100vw" /> </div> {car.imageUrls.length > 1 && <> <button onClick={() => setFullscreenImageIndex((prev) => (prev - 1 + car.imageUrls.length) % car.imageUrls.length)} className="absolute left-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-black/60 hover:bg-black/80 backdrop-blur-sm rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 z-40" aria-label="Предыдущее фото"> <ChevronLeft className="h-8 w-8 text-white" /> </button> <button onClick={() => setFullscreenImageIndex((prev) => (prev + 1) % car.imageUrls.length)} className="absolute right-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-black/60 hover:bg-black/80 backdrop-blur-sm rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 z-40" aria-label="Следующее фото"> <ChevronRight className="h-8 w-8 text-white" /> </button> </>} <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40"> <div className="bg-black/60 backdrop-blur-sm rounded-full px-4 py-2 text-white text-sm font-medium">{fullscreenImageIndex + 1} из {car.imageUrls.length}</div> </div> {car.imageUrls.length > 1 && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 hidden md:flex space-x-2 z-40 max-w-4xl overflow-x-auto scrollbar-hide pb-1"> <div className="flex space-x-2 px-2"> {car.imageUrls.map((url, index) => <button key={index} onClick={() => setFullscreenImageIndex(index)} className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-200 ${index === fullscreenImageIndex ? 'border-white shadow-lg scale-105' : 'border-white/30 hover:border-white/60 hover:scale-102'}`}> <Image src={getCachedImageUrl(url)} alt={`${car?.make} ${car?.model} - миниатюра ${index + 1}`} width={64} height={64} quality={60} sizes="64px" className="w-full h-full object-cover" /> </button>)} </div> </div>} {car.imageUrls.length > 1 && <div className="absolute bottom-20 left-1/2 -translate-x-1/2 md:hidden z-40"> <div className="flex space-x-2"> {car.imageUrls.map((_, index) => <button key={index} onClick={() => setFullscreenImageIndex(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === fullscreenImageIndex ? 'bg-white shadow-lg scale-125' : 'bg-white/50 hover:bg-white/75'}`} />)} </div> </div>} </div>} </DialogContent> </Dialog>
      </div>
    </div>
  )
}