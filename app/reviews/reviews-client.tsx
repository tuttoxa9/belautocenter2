...
export default function ReviewsClient({ initialReviews }: ReviewsClientProps) {
  const [reviews, setReviews] = useState<Review[]>(initialReviews.map(r => ({...r, createdAt: new Date(r.createdAt)})))
  const [loading, setLoading] = useState(initialReviews.length === 0)
  const [filteredReviews, setFilteredReviews] = useState<Review[]>(initialReviews.map(r => ({...r, createdAt: new Date(r.createdAt)})))
