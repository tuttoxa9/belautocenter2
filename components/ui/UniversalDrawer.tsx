    ...
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side={side} className={`w-[600px] sm:max-w-[800px] p-0 flex flex-col ${className}`}>
          ...
          {footer && (
            <SheetFooter className="p-4 sm:p-6 border-t bg-background/95 backdrop-blur-sm">
              {footer}
            </SheetFooter>
          )}
        </SheetContent>
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} direction={direction}>
      <DrawerContent direction={direction} className={`${maxHeight} flex flex-col ${className}`}>
        ...
        {footer && (
          <DrawerFooter className="flex-shrink-0 border-t mt-auto bg-background/95 backdrop-blur-sm">
            {footer}
          </DrawerFooter>
        )}
      </DrawerContent>
