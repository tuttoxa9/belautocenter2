...
async function handleGetCars(request, env, ctx) {
    ...
    // Проверяем заголовки Cache-Control от клиента
    const cacheControl = request.headers.get('Cache-Control');
    const shouldBypassCache = cacheControl && (cacheControl.includes('no-cache') || cacheControl.includes('no-store'));

    if (shouldBypassCache) {
        console.log('Bypassing cache for /cars due to Cache-Control header');
        const response = await fetch(firestoreRequest);
        const freshResponse = new Response(response.body, response);
        freshResponse.headers.set("X-Cache-Status", "BYPASS");
        freshResponse.headers.set('Access-Control-Allow-Origin', '*');
        return freshResponse;
    }
...
async function handleUpload(request, env) {
  ...
  try {
    ...
    await env.IMAGE_BUCKET.put(path, file.stream(), { httpMetadata: { contentType: file.type } });
