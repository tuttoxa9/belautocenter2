// Cloudflare Worker для BelAutoCenter – УНИВЕРСАЛЬНАЯ ВЕРСИЯ v2.1
// Поддерживает: R2 (с конвертацией в WebP при загрузке) + Кэширующий прокси для Firestore.
...
async function proxyFirestore(request, env, ctx) {
    ...
    if (response.ok) {
        ...
        const parts = url.pathname.split('/').filter(p => p); // /cars/123 -> ['cars', '123']
    ...
    // Возвращаем оригинальный ответ от Firestore без кэширования для мутаций
